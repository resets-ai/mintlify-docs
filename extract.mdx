...
<Note>
  **No charge on failure** - In most cases you are only charged credits when extraction successfully returns data. Some services do charge a small amount even if no content is extracted. We try to limit this as much as possible.
</Note>
