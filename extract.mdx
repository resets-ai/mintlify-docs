...
<Info>
  **Smart Default Prompt**
  ...
  When you provide only a schema without a prompt, we automatically generate: "You are an experienced data extraction analyst, extract data from the provided content using the provided json schema. If information for a field is not available leave it as empty"
</Info>
...
3. **Sequential Attempts** - Tries services in order (Jina → EXA → Fallback)
...
- **Protected Content** - Some authentication-required pages
...
| Failed extraction   | 0 credits |

<Note>
  **No charge on failure** - You're only charged credits when extraction successfully returns data
</Note>
